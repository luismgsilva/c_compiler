...
static bool op_treadted_as_on (char op)
{
    return op == '(' ||
        op == '{' ||
        op == ',' ||
...
/*
 * Read multiple operators to tread them as a one
 * For example: if there are "++", the first "+" is
 * read, but since the next one is also "+", they will
 * by treated as a single operator.
 */
const char
*read_op ()
{
...
*token_make_one_line_comment ()
{
    ...
    /*
     * Example: // Hello world
     * It should be read until a newline or EOF character is found.
     */
...
*handle_comment ()
{
    ...
    if (c == '/')
    {
        ...
        /*
         * If its neither of the above,
         * then we are dealing with a division operator.
         */
        pushc('/');
...
*token_make_special_number ()
{
    ...
    /*
     * Example: 0x500
     * Popping the first 0, because it is not used
     * besides to indicate that its a special number
     */
    lexer_pop_token();
...
*read_next_token()
{
    ...
    switch (c)
    {
        ...
        /*
         * Excluding divison because '/' can also
         * be a start of a comment.
         */
...
char
lexer_string_buffer_next_char (struct lex_process *process)
{
...
char
lexer_string_buffer_peek_char (struct lex_process *process)
{
    struct buffer *buf = lex_process_private(process);
...
void
lexer_string_buffer_push_char (struct lex_process *process, char c)
{
    struct buffer *buf = lex_process_private(process);
    buffer_write(buf, c);
}
...
/*
 * This function allows to use the lexical analysis
 *   functionality without having to restrict only to the input file.
 * Can pass actual strings, which will then function as a input file.
 */
struct lex_process*

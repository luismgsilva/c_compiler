...
bool
token_is_nl_or_comment_or_newline_seperator (struct token *token)
{
    if (!token)
    {
        return false;
...
bool
token_is_operator (struct token* token, const char* val)
{
    return token && token->type == TOKEN_TYPE_OPERATOR && S_EQ(token->sval, val);
}

bool
token_is_primitive_keywords (struct token* token)
{

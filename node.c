...
void
make_union_node (const char* name, struct node* body_node)
{
    int flags = 0;
    if (!body_node)
    {
...
struct node*
union_node_for_name (struct compile_process* current_process, const char* name)
{
...
bool
node_is_expression (struct node* node, const char* op)
{
    return node->type == NODE_TYPE_EXPRESSION && S_EQ (node->exp.op, op);
}

bool
is_array_node (struct node* node)
{
    return node_is_expression (node, "[]");
}

bool
is_node_assignment (struct node* node)
{
